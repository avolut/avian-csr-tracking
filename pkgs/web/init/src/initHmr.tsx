...
export const initHmr = () => {
  ...
  if (ws) {
    ...
    ws.onConnected = () => {
      ...
      const initTime = ellapsedTime(window.tstamp)
      if (initTime > 1) {
        console.warn(
          `[BASE] Init Warning:

  Your Browser (Firefox/Chrome) extension adds ${initTime}s to live-reload.
  Disable them to improve development experience.`
        )
      }
    }
