...
export const upgradePageV2 = async (path: string) => {
  const fullPath = join(dirs.app.web, "src", "base", "page", path);
  const { effect, parsed, source } = await parseEffect(fullPath);

  let run = effect.run;
  if (run.startsWith("()")) {
    run = `({meta }) ${run.substring(2)}`;
  } else if (run.startsWith("async ()")) {
    run = `async ({meta }) ${run.substring("async ()".length)}`;
  }
...
base({
  meta: ${effect.meta},
  init: ${run}
}, ({meta}) => (${trim(generate(parsed).code, ";")}))`,
    {
      parser: "babel-ts",
    }
  );
