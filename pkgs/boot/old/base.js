...
const basePull = async () => {
  const dir = join(os.tmpdir(), 'andro-base-pull')
...
const basePush = async () => {
  const dir = join(os.tmpdir(), 'andro-base-push')

  let commitMsg = args._.slice(1).join(' ')
  if (!commitMsg) {
    console.log(`Please provide commit message, 
 e.g.
   npm run base push fixing something...    `)
    return
  }
