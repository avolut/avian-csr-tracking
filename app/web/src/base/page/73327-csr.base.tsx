        ...
        csr: {
          ...
          form: {
            ...
            onSave: async ({ data, save }) => {
              ...
              if (typeof data.is_training === 'string')
                data.is_training = Number(data.is_training)
              if (typeof data.value_biaya === 'string')
                data.value_biaya = Number(data.value_biaya)
              save()
            },
            params: {
              include: {
                      ...
                      columns: [
                        ...
                        [
                          'id_supplier',
                          {
                            ...
                            value: (item) => {
                              return (
                                <span>{item.m_supplier?.nama_supplier}</span>
                              )
                            },
                          },
                        ],
                    ...
                    alter: {
                      bantuan: {
                        ...
                        onChange: (value, { state }) => {
                          if (
                            value === 'Cat' &&
                            !!state.config.fields.m_jenis_bantuan
                          ) {
                      ...
                      m_cabang: {
                        ...
                        params: (row) => {
                          if (
                            !row.id_area_tirta &&
                            row.id_supplier != 'PT Tirtakencana Tatawarna'
                          )
                            return {}
                          return {
                            where: {
                        ...
                        if (row.bantuan === 'Cat') {
                          return layout([
                            ...
                            ({ row, watch, layout }) => {
                              watch(['m_supplier'])
                              if (
                                row?.m_supplier?.nama_supplier ===
                                'PT Tirtakencana Tatawarna'
                              )
                                return layout([['m_area_tirta', 'm_cabang']])
                    ...
                    alter: {
                      jenis_biaya: {
                        ...
                        onChange: (value, { state }) => {
                          if (
                            value !== 'Lainnya' &&
                            !!state.config.fields.m_supplier
                          ) {
                ...
                fieldProps: {
                  ...
                  form: {
                    onSave: async ({ data, save, state }) => {
                      const csr = await db.t_csr.findFirst({
                        where: { id: state.tree.parent.tree.parent.db.data.id },
                      })
                      if (csr?.status === 'Draft') {
            ...
            layout: [
              ...
              ({ row, watch, update, layout, state }) => {
                ...
                if (
                  !!row.m_jenis_instansi &&
                  row.m_jenis_instansi.jenis_instansi === 'Sebutkan'
                )
                  return layout([['keterangan', 'jumlah_orang']])
